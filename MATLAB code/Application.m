...
prompt='Type 1 for new parameter setup; 2 for last parameter setup. Press Enter after typing.';
...
while flag_prog==0
    ...
    ocrresult=ocr(dilatepaint,'TextLayout','Block','CharacterSet','CPN');
    if ocrresult.WordConfidences>0.6
        if ocrresult.Words{1} == 'P'
            flag=1;
            fwrite(s,201);
            fprintf('p');
        elseif ocrresult.Words{1} == 'C'
            flag=0;
            fwrite(s,200);
            fprintf('c');
